def main():
    ...
    print("終了するにはescapeキーを押してください")
    cap = cv2.VideoCapture(0)
    bg_cap = cv2.VideoCapture(os.path.join(module_path, bg_filename))
    with mp_selfie_segmentation.SelfieSegmentation(
        ...
        while cap.isOpened():
            ...
            if not success:
                ...
                continue
            success, bg_image = bg_cap.read()
            if not success:
                bg_cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                _, bg_image = bg_cap.read()
                # If loading a video, use 'break' instead of 'continue'.
